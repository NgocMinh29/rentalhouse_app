...
//
//    }
        private ObservableList<KhachData> KhachNHList;

    public void KhachNganXuat(ActionEvent event) {
        KhachNHList = khachngan_tableview.getItems();
        
        try {
            // Kiểm tra nếu danh sách rỗng
            if (KhachNHList == null || KhachNHList.isEmpty()) {
            ...
            XSSFSheet sheet = workbook.createSheet("Khách ngắn hạn");
            XSSFRow row = sheet.createRow(1);
            Cell cell;

            String[] headers = {"MAKT", "Họ tên", "Giới tính", "Ngày sinh", "SĐT", "CCCD", "Email", "Mã người đại diện", "Ngày bắt đầu", "Ngày kết thúc", "Trạng thái"};
            ...
            for (int i = 0; i < KhachNHList.size(); i++) {
                KhachData khach = KhachNHList.get(i);
                ...
                cell.setCellValue(khach.getDaidienidProperty().get());
                ...
                cell.setCellValue(khach.getTrangthaiProperty().get());
            }

            File file = new File("Danh sách Khách ngắn hạn.xlsx");
